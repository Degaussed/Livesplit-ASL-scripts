state ("MirrorsEdge", "Steam / GOG") //Taken from Toyro's Chapter Autosplitter
{
    //float IGT : 0x01C55EA8, 0xBC, 0x0, 0x28, 0x114; hk old
    //float IGT : 0x01C553D0, 0xCC, 0x1CC, 0x2F8, 0x9C, 0xCC4, 0x368, 0x114; toyro's value is showing null sometimes
    float IGT : 0x01B73F1C, 0xCC, 0x1CC, 0x698;
    string50 level : 0x01BF8B20, 0x3CC, 0x0;                //TdMainMenu > tutorial_p > Edge_p
    string50 chkpt : 0x01C55EA8, 0x74, 0x0, 0x3c, 0x0;      // ?? > start > Edge_Start
    int progAny : 0x01B73F1C, 0xCC, 0x570;
    int prog69 : 0x01B73F1C, 0xCC, 0x18, 0x8, 0x5E0;
}

state ("MirrorsEdge", "Origin")
{
    //float IGT: 0x01C6E50C, 0xCC, 0x1CC, 0x2F8, 0x9C, 0xCC4, 0x368, 0x114; toyro
    //float IGT : 0x01BB166C, 0x0, 0x18, 0x48, 0x28, 0x114; hk old
    float IGT : 0x01C6E50C, 0xCC, 0x1CC, 0x698;
    string50 level : 0x01C11BE0, 0x330, 0x0;
    string50 chkpt : 0x01C6EFE0, 0x134, 0xBC, 0x0, 0x3C, 0x0;
    int progAny : 0x01C6065C, 0x74, 0x338, 0x570;
    int prog69 : 0x01C5ECC8, 0x31C, 0xCE0, 0x338, 0x5E0;
}
...
state ("MirrorsEdge", "Reloaded")
{
    ...
    string50 chkpt : 0x01C6EFE0, 0x134, 0xBC, 0x0, 0x3C, 0x0;
    int progAny : 0x01C14C14, 0x74, 0x338, 0x570;
    int prog69 : 0x01C6EFE0, 0x3C, 0x58, 0x70, 0x0, 0x338, 0x5E0;
}

state ("MirrorsEdge", "Origin DLC") // this version sucks ass and won't be supported probably
{
    float IGT : 0x01BB166C, 0x0, 0x18, 0x48, 0x28, 0x114;
    string50 level : 0x01C18C94, 0x330, 0x0;
    string50 chkpt : 0x01C76108, 0x74, 0x0, 0x3C, 0x0;
}
...
startup
{
    ...
    settings.Add("69", false, "69 Stars autosplitter, splits on final checkpoint");
    settings.Add("sd", false, "Optional Split at Stormdrains Exit Button");
}

update
{
    //print("buh " + current.prog69);
}
...
split
{
    ...
    //if (settings["69"] && current.prog69 == 969696 && old.prog69 != current.prog69) return true;
    if (settings["69"] && current.prog69 == old.prog69 + 1 && old.prog69 != null) return true;
...
start
{
    if (current.level == "tutorial_p" && current.chkpt == "start" && current.IGT > 0) return true;
    if (settings["69"] && current.level == "tt_TutorialA01_p" && old.IGT == 0 && current.IGT > 0) return true;
}

reset
{
    if (current.level == "tutorial_p" && current.chkpt == null && current.IGT == 0) return true;
    //if (settings["69"] && current.level == "tt_TutorialA01_p" && current.prog69 == 0 && old.prog69 != current.prog69) return true;
}
